    ...
    <div style={panelStyle}>
      ...
      <div style={metricsRowStyle}>
        ...
        <MetricBadge
          ...
          color={
            node.expanded ? "#00f084" : node.hasChildren ? "#ffc145" : "#6b7a9d"
          }
        />
      </div>

      <Divider
        style={{ margin: "10px 0", borderColor: "rgba(255,255,255,0.05)" }}
      />
      ...
      <div style={scrollAreaStyle}>
        ...
        {node.hasChildren && (
          <div style={{ padding: "0 14px 12px" }}>
            <Tooltip
              title={
                node.expanded ? "Children already loaded" : "Load child nodes"
              }
            >
              <Button
                block
                size="small"
                icon={
                  isExpanding ? <LoadingOutlined spin /> : <ExpandAltOutlined />
                }
        ...
        {outgoing.length > 0 && (
          <section style={sectionStyle}>
            <SectionHeader
              icon="→"
              label={`CALLS / USES  (${outgoing.length})`}
            />
            <div
              style={{
                display: "flex",
            ...
            >
              {outgoing.slice(0, 20).map(({ edge, peer }) => (
                <EdgeRow
                  key={edge.id}
                  edge={edge}
                  peer={peer}
        ...
        {incoming.length > 0 && (
          <section style={sectionStyle}>
            <SectionHeader
              icon="←"
              label={`CALLERS / DEPS  (${incoming.length})`}
            />
            <div
              style={{
                display: "flex",
...
const PropertyRow: React.FC<{ propKey: string; value: string }> = ({
  propKey,
  value,
}) => (
  <div
...
function getConnectedEdges(
  ...
  incoming: { edge: EngineGraphEdge; peer: EngineGraphNode | null }[];
  outgoing: EdgeEntry[];
} {
