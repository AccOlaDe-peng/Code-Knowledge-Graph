...
const NAV_ITEMS: NavItem[] = [
  { path: '/',              icon: <IconDashboard size={18} />, label: '概览', description: '系统仪表盘' },
  { path: '/repository',    icon: <IconRepository size={18} />, label: '仓库', description: '代码仓库管理' },
  { path: '/architecture',  icon: <IconArchitecture size={18} />, label: '架构图', description: '模块架构视图' },
  { path: '/callgraph',     icon: <IconCallGraph size={18} />, label: '调用图', description: '函数调用关系' },
  { path: '/lineage',       icon: <IconDataLineage size={18} />, label: '数据血缘', description: '数据流向追踪' },
  { path: '/fieldlineage',  icon: <IconFieldLineage size={18} />, label: '字段血缘', description: '字段级数据流转' },
  { path: '/eventflow',     icon: <IconEventFlow size={18} />, label: '事件流', description: '事件订阅关系' },
  { path: '/query',         icon: <IconQuery size={18} />, label: 'AI 查询', description: '智能代码问答' },
  { path: '/impact',        icon: <IconImpactAnalysis size={18} />, label: '影响分析', description: '变更影响评估' },
];
...
const NavItemComponent: React.FC<{
  ...
  return (
    ...
    >
      {/* Ripple effects */}
      {ripples.map((ripple) => (
        <span key={ripple.id} style={getRippleStyle(ripple, 'rgba(0, 212, 255, 0.25)')} />
      ))}
