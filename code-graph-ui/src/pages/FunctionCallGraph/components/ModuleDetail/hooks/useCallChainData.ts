...
interface UseCallChainDataResult {
  ...
  collapsedInfo: Map<
    string,
    { count: number; distance: number; direction: "caller" | "callee" }
  >;
  hasCycle: boolean;
...
function traverseCallChain(
  ...
  const edges = new Map<
    string,
    { edge: CallChainEdgeData; chain: CallChain }
  >();
  ...
  for (const chain of rootCallers) {
    ...
    if (!edges.has(edgeId)) {
      edges.set(edgeId, {
        edge: {
          sourceFunctionId: callerId,
          targetFunctionId: rootId,
  ...
  for (const chain of rootCallees) {
    ...
    if (!edges.has(edgeId)) {
      edges.set(edgeId, {
        edge: {
          sourceFunctionId: rootId,
          targetFunctionId: calleeId,
          sourceLine: chain.sourceLine,
          callType: chain.callType,
          isCrossModule: chain.sourceModuleId !== chain.targetModuleId,
        },
  ...
  while (queue.length > 0) {
    ...
    if (visited.has(id)) {
      ...
      // 标记循环边
      const cycleEdgeId =
        direction === "caller" ? `${id}->${rootId}` : `${rootId}->${id}`;
    ...
    if (
      distance > maxDepth &&
      !collapsedNodes.has(`collapsed-${direction}-${distance}`)
    ) {
      // 统计折叠节点
      const collapseKey = `collapsed-${direction}-${distance}`;
      collapsedCounts.set(
        collapseKey,
        (collapsedCounts.get(collapseKey) || 0) + 1,
      );
      continue;
    }
    ...
    if (func) {
      ...
      if (
        distance < maxDepth ||
        collapsedNodes.has(`collapsed-${direction}-${distance + 1}`)
      ) {
