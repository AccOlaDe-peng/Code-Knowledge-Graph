<template>
  <div class="graph-view">
    <div class="breadcrumb">
      ...
      <span class="breadcrumb-separator">/</span>
      <span class="breadcrumb-item active">业务流程图</span>
    </div>

    <div v-loading="loading" class="graph-container-wrapper">
      <div class="graph-header">
        <div class="graph-title-block">
          <h1 class="graph-title">业务流程图</h1>
          <p class="graph-subtitle">按架构层次展示 Controller → Service → Repository 调用链</p>
        </div>
        <div class="graph-header-right">
          <div v-if="nodeStats.total > 0" class="node-stats">
            ...
            <span v-else class="stats-info">
              共 {{ nodeStats.total }} 个节点，{{ nodeStats.edges }} 条调用关系
            </span>
          </div>
      ...
      <div v-if="isEmpty" class="empty-state">
        ...
        <p>未检测到明确的分层架构</p>
        <p class="empty-hint">请确保代码使用 NestJS 装饰器（@Controller、@Injectable 等）或文件命名约定（*Controller.ts、*Service.ts）</p>
      </div>
...
const legendItems = [
  { label: 'Controller', color: '#58a6ff' },
  { label: 'Service', color: '#3fb950' },
  { label: 'Repository', color: '#e3b341' },
  { label: 'Model/DTO', color: '#8b949e' },
]
...
const loadGraphData = async () => {
  ...
  try {
    ...
    const response = await graphApi.getBusinessFlow(projectId)
    ...
    ElMessage.error(error.response?.data?.message || '加载业务流程图失败')
  } finally {
    loading.value = false
  }
}
...
const initGraph = async (
  ...
  graph = new Graph({
    ...
    data: {
      nodes: nodes.map(n => ({
        id: n.id,
        data: { label: n.label, layer: n.layer, endpoints: n.endpoints },
      })),
      edges: edges.map((e, i) => ({
        id: `e${i}`,
        source: e.source,
    ...
    node: {
      style: (d: any) => {
        const endpoints: Array<{ method: string; path: string }> = d.data?.endpoints || []
        const epStr = endpoints
          .slice(0, 2)
          .map((e: any) => `${e.method} ${e.path}`)
          .join('\n')
        const baseLabel = d.data?.label || d.id
        return {
          size: 36,
          fill: LAYER_COLORS[d.data?.layer] || LAYER_COLORS.unknown,
          stroke: LAYER_COLORS[d.data?.layer] || LAYER_COLORS.unknown,
          lineWidth: 2,
          labelText: epStr ? `${baseLabel}\n${epStr}` : baseLabel,
          labelFill: '#e6edf3',
          labelFontSize: 10,
    ...
    layout: { type: 'dagre', rankdir: 'TB', nodesep: 24, ranksep: 100, controlPoints: false },
  ...
  graph.on('node:click', (evt: any) => {
    ...
    const layerLabel: Record<string, string> = {
      ...
      repository: 'Repository 层', model: 'Model/DTO', unknown: '未知层'
    }
    ElMessage.info(`${nodeData.label || nodeId}（${layerLabel[nodeData.layer] || ''}）`)
  })
}
...
.empty-hint { font-size: 13px !important; max-width: 480px; line-height: 1.6; }
